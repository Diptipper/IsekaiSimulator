...
class CharacterMenu {
    ...
    // HP Formula: 10 + (Vitality Level * 5)
    getMaxHP() { 
        let b=10+(this.getSkillLevel("vitality")*5); 
        let bo=0;
        for(let s in this.equipment)
            if(this.equipment[s])
                bo+=this.equipment[s].bonus_hp;
        return b+bo; 
    }
    ...
    getDisplayHTML() {
        ...
        // 4. SKILLS WITH BARS
        const skillList = ["strength", "defense", "vitality", "agility", "mining", "woodcutting", "smithing"];
...
const Game = {
    ...
    showMainMenu: function() {
        ...
        safelySetBackgroundImage('scene-bg-container', 'scene-image', "data/images/bg/", "title_screen.png");
    ...
    startDialogue: function(dialogueObj, callback) {
        ...
        safelySetBackgroundImage('dialogue-bg-container', 'dialogue-bg', "data/images/bg/", dialogueObj.background);
        ...
        if (dialogueObj.speakers) {
            for (let speakerName in dialogueObj.speakers) {
                ...
                img.src = "data/images/sprites/" + filename;
    ...
    updateSkillHUD: function(skillName, duration = 0) {
        ...
        // Clear existing timer to prevent premature hiding if spamming XP
        if (this.skillBarTimeout) clearTimeout(this.skillBarTimeout);
        
        // If a duration is set, hide after X ms
        if (duration > 0) {
            this.skillBarTimeout = setTimeout(() => {
                container.style.display = 'none';
            }, duration);
        }
    },
    ...
    render: function() {
        ...
        safelySetBackgroundImage('scene-bg-container', 'scene-image', "data/images/bg/", this.currentImage);
        this.updatePlayerHUD(); 
    ...
    executeCombatTurn: function(participants, index, enemies, roundInterval) {
        ...
        if (this.player.hp <= 0) {
            ...
            this.waitForPlayer(() => {
                if (this.activeBinding)
                    GLOBAL_STATE.variables[this.activeBinding] = "false";
                this.stopAction(); this.player.hp = 1; this.updatePlayerHUD(); });
            return;
        }
        ...
        if (potentialTargets.length > 0) {
            ...
            // 2. Determine Scaling (Weapon Type)
            let scalingStat = "strength"; // Default
            let scalingValue = actorSkills.strength;

            // If actor is player, check equipped weapon scaling
            if (actor === this.player && this.player.equipment.weapon) {
                // If defined in data.js, use it, otherwise default strength
                if (this.player.equipment.weapon.scaling === "agility") {
                    scalingStat = "agility";
                    scalingValue = actorSkills.agility;
                }
            } 
            // If actor is enemy, simple AI: use their highest stat
            else if (actor !== this.player) {
                if (actorSkills.agility > actorSkills.strength) {
                    scalingStat = "agility";
                    scalingValue = actorSkills.agility;
                }
            }
            ...
            // 3. Calculate Damage
            // Raw force is the Stat + Bonus (Weapon bonus is already in actorSkills)
            const rawDamage = scalingValue;
            const defense = targetSkills.defense;
